...
可以概括为：`main.tsx` 负责装配，`query.ts` 负责推进，周边模块提供能力、状态和约束。
...
因此，这里不是“采样 -> 工具 -> 返回”的线性流水线，而是带恢复分支的 turn state machine。
...
因此，Claude Code 的 autonomy boundary 不在 UI，而在 runtime pipeline 本身。
...
因此，整套系统更像可持续演化的产品 runtime，而不是本地小工具。
